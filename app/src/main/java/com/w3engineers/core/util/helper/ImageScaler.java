...
/*			options.inPurgeable = true;
    ...
    private String getFilename(Type type, String extension, boolean isToUseTime) {
        ...
        if (type == Type.IMAGE) {
            ...
            fileFolder = DirConst.TEMP_DIRECTORY;
            prefix = "img_";
    ...
    public Bitmap getThumnailImage(ImageTier tier, String imagePath) {
         ...
         */
        boolean isPanorama = ImageScaler.onScaler(AppController.getContext()).getIsPanorama(actualWidth, actualHeight);
        ...
        options.inSampleSize = ImageScaler.onScaler(AppController.getContext())
                .calculateInSampleSize(options, ImageScaler.onScaler(AppController.getContext())
                        .convertDipToPixels(actualWidth), ImageScaler.onScaler(AppController.getContext()).convertDipToPixels(actualHeight));
        options.inJustDecodeBounds = false;
        options.inDither = false;
/*			options.inPurgeable = true;
        options.inInputShareable = true;*/
    ...
    public String prepareThumbString(@NonNull String path, int height, int width) {
        ...
        Bitmap bitmap = getThumnailImage(ImageTier.SHARE, path);
...
//          write the compressed bitmap at the destination specified by filename.
    ...
    public String getCompressedImagePath(String extension) {
        File file = new File(DirConst.TEMP_DIRECTORY);
        if (!file.exists()) {
            file.mkdirs();
        }
        String uriSting = (file.getAbsolutePath() + "/" + TimeUtil.currentTime() + "." + extension);
        Logger.log("FileSize", "Comptressed apth =" + uriSting);
        return uriSting;
